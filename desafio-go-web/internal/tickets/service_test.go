...
func TestGetTotalTickets(t *testing.T) {
	...
	t.Run("Should return not nil avg", func(t *testing.T) {
		dbMock := &DbMock{
			db:  tickets,
			spy: false,
			err: nil,
	...
	t.Run("Should return error inexist country", func(t *testing.T) {
		country := "Brazil"
		dbMock := &DbMock{
			db:  tickets,
			spy: false,
		...
		service := NewService(repo)
		expect := 0.0
		avg, err := service.AverageDestination(cxt, country)
		assert.NotNil(t, err)
		assert.Equal(t, expect, avg)
		assert.True(t, dbMock.spy)
	})
